...
void CPasswordInputField::updatePassword() {
    ...
    if (passwordLength != password.text.passwordLength)
        password.show = false;
}
...
void CPasswordInputField::renderPasswordUpdate() {
    ...
    if (newAsset) {
        ...
        password.text.resourceID = "";
        std::swap(password.text.resourceID, password.text.pendingResourceID);
    } else {
        Debug::log(WARN, "Asset {} not available after the asyncResourceGatherer's callback!", password.text.resourceID);
...
bool CPasswordInputField::draw(const SRenderData& data) {
    ...
    if (!hiddenInputState.enabled) {
        ...
        const double PAD = (inputFieldBox.h - passSize.y) / 2.0;

        if ((passwordLength > 0 || checkWaiting) && password.allowToggle && eye.closedAsset) {
            const auto EYESIZE = passSize * 1.25;
            const auto EYEPAD  = (inputFieldBox.h - EYESIZE.y) / 2.0;
            inputFieldBox.w -= (EYESIZE.x + 2 * PAD);
            // TODO: placement options
            // TODO: padding not ideal
            eye.box = CBox{inputFieldBox.pos() + Vector2D{inputFieldBox.w + PAD, EYEPAD}, EYESIZE};
            g_pRenderer->renderTexture(eye.box, (password.show) ? eye.openAsset->texture : eye.closedAsset->texture, fontCol.a);
        }

        const auto   CURRDOTS     = dots.currentAmount->value();
        const double DOTAREAWIDTH = inputFieldBox.w - (PAD * 2);
        ...
        double xstart = dots.center ? ((DOTAREAWIDTH - CURRWIDTH) / 2.0) + PAD : PAD;

        if (password.show && password.text.asset) {
            const Vector2D ASSETPOS = inputFieldBox.pos() + inputFieldBox.size() / 2.0 - password.text.asset->texture.m_vSize / 2.0;
            const CBox     ASSETBOX{ASSETPOS, password.text.asset->texture.m_vSize};

            // TODO: can we make the texture slightly fade out towards the ends like the dots?
            glEnable(GL_SCISSOR_TEST);
            glScissor(inputFieldBox.x, inputFieldBox.y, inputFieldBox.w, inputFieldBox.h);
            g_pRenderer->renderTexture(ASSETBOX, password.text.asset->texture, data.opacity * fade.a->value(), 0);
            glScissor(0, 0, viewport.x, viewport.y);
            glDisable(GL_SCISSOR_TEST);
            return false;
        }
...
CBox CPasswordInputField::getEyeBoxWl() {
    return {
        Vector2D{eye.box.pos().x, viewport.y - eye.box.pos().y - eye.box.size().y},
        eye.box.size(),
    };
}

void CPasswordInputField::onHover(const Vector2D& pos) {
    if (!password.allowToggle || passwordLength == 0)
        return;

    CBox eyeBox = getEyeBoxWl();
...
void CPasswordInputField::onClick(uint32_t button, bool down, const Vector2D& pos) {
    if (!password.allowToggle || passwordLength == 0 || !down)
        return;
    ...
    if (eyeBox.containsPoint(pos)) {
        ...
        password.text.passwordLength = g_pHyprlock->getPasswordBufferLen();

        Debug::log(TRACE, "Toggle password visibility to {}", password.show);
        if (!password.show) {
        ...
        Debug::log(TRACE, "Requesting pw asset with resourceID {}", textResourceID);
        if (!password.text.pendingResourceID.empty()) {
            Debug::log(WARN, "Pending pw asset. Refusing to request another one");
            return;
        }

        if (password.text.resourceID == textResourceID)
            return;
