...
#define SHADOWABLE(name)                                                                                                                                                           \
    ...
    for (auto& k : keys) {
        ...
        result.push_back(CConfigManager::SWidgetConfig{
            "session-picker",
            std::any_cast<Hyprlang::STRING>(m_config.getSpecialConfigValue("session-picker", "monitor", k.c_str())),
            {
...
std::vector<SLoginSessionConfig> CConfigManager::getLoginSessionConfigs() {
    ...
    for (auto& k : keys) {
        result.push_back(SLoginSessionConfig{
            std::any_cast<Hyprlang::STRING>(m_config.getSpecialConfigValue("login-session", "name", k.c_str())),
            std::any_cast<Hyprlang::STRING>(m_config.getSpecialConfigValue("login-session", "exec", k.c_str())),
        });
    }
